...
from backend.models import ParkingLot, ParkingHistory, User, db, Role 
from backend.celery.mail_service import send_email

# Add render_template to your imports from flask
from flask import render_template
# Add Counter for an easy way to find the most common item
from collections import Counter
...
def send_daily_reminders():
    """
    Sends a daily reminder to users who have NOT parked in the last 7 days.
    """
    # 1. Define the time threshold (7 days ago)
    seven_days_ago = datetime.utcnow() - timedelta(days=7)

    # 2. Find users who HAVE parked recently
    # This subquery finds all user IDs that have a ParkingHistory record
    # more recent than seven_days_ago.
    ...
    # 3. Find users who are NOT in that recent list
    # We select users whose ID is NOT IN the subquery of recent users.
...
def send_monthly_reports():
    ...
    for user in users:
        ...
        lot_ids = [item.spot.lot_id for item in history]
        ...
        summary_data = {
            ...
            "most_used_lot": most_used_lot.name if most_used_lot else "N/A"
        }

        # Use render_template to generate the detailed report
