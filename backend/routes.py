...
def register():
    ...
    # Check if a user with that email or vehicle number already exists
    if datastore.find_user(email=email):
        return jsonify({"message": "User with this email already exists"}), 409
    from .models import User  # Make sure User is imported if not already
    ...
    try:
        # Create the user with all the required fields
